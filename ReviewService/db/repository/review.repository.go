...
type ReviewRepository interface {
	...
	Create(userId, bookingId, hotelId int64, comment string, rating int) (*models.Review, error)
...
func (r *ReviewRepositoryImpl) GetAll() ([]*models.Review, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&review.Id,&review.UserId,&review.BookingId,&review.HotelId,&review.Comment,&review.Rating,&review.CreatedAt,&review.UpdatedAt,&review.DeletedAt,&review.IsSynced); err!= nil{
			fmt.Println("Error in scanning reviews",err)
			return  nil,err
		}
...
func (r *ReviewRepositoryImpl) GetById(id int64) (*models.Review,error){
	...
	if err!= nil{
		if err == sql.ErrNoRows{
			...
			fmt.Println("error in scanning review")
			return  nil,err
		}
	}
...
func (r *ReviewRepositoryImpl) Create(userId, bookingId, hotelId int64, comment string, rating int) (*models.Review, error) {
	...
	if err != nil{
		fmt.Println("error in creating reviews",err)
			return  nil,err
	}

	lastInsertedID,rowErr := result.LastInsertId()
	if rowErr != nil{
		fmt.Println("error getting last insrted id",rowErr)
		return  nil,rowErr
	}
...
func (r *ReviewRepositoryImpl) Update(id int64, comment string, rating int) (*models.Review, error){
	...
	if err != nil{
		fmt.Println("error in updating reviews",err)
			return  nil,err
	}
	...
	if rowsAffected == 0{
		fmt.Println("no rows are affected, review not found or already deleted")
		return nil, fmt.Errorf("review not found")
	}

	return  r.GetById(id) // fetch the updated review
}
...
func (r *ReviewRepositoryImpl) GetByHotelId(hotelId int64) ([]*models.Review, error){
	...
	if err != nil {
		fmt.Println("error fetching Hotel ID",err)
		return nil,err
	}
	defer rows.Close()
...
func (r *ReviewRepositoryImpl) GetByBookingId(bookingId int64) ([]*models.Review, error) {
	...
	if err != nil {
		fmt.Println("error fetching Booking ID",err)
		return nil,err
	}
	defer rows.Close()
